...
// const TOKEN = 'uf3kQLmgBKG40YH2OreqjxRXwlIJnsME'; // token,搜石
const TOKEN = 'd9c55ad403bd46a38a736141addabbe7'; // token,搜石
Page({
    ...
    getQuery: function () {
        App.get(App.api.goodsQuery, { merchantId: this.data.merchantId, token: TOKEN, variety: this.data.variety }).then(result => {
            ...
            const queryList = result.splice(0, 10)
            this.setData({
                queryList: queryList
            })
            // 用于分享出去，设置编号
    ...
    detailCodeTab: function (blockId) {
        App.get(App.api.detailTab, { id: blockId, token: TOKEN, tab: 2 }).then(result => {
            ...
            let swiperHeight = (codeInfo[0].slicesInfo.length + 2) * 90 + 'rpx'
    ...
    detailHdTab: function (blockId) {
        App.get(App.api.detailTab, { id: blockId, token: TOKEN, tab: 3 }).then(result => {
            ...
            // 初始化当前扎的高清图列表
            console.log(this.data.hdActive)
            this.getPreviewHd(this.data.hdActive)
